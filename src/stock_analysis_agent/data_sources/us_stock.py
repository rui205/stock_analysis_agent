"""US equity data source — wraps yfinance.

yfinance is an unofficial Yahoo Finance API wrapper. It's fine for
personal/research use but can rate-limit or return partial data silently.
The data source layer validates responses and wraps upstream failures in
our typed exception hierarchy.

Data-source attribution: https://github.com/ranaroussi/yfinance
"""
...
def _yfinance_call(func, *args, **kwargs):
    """Run a yfinance call and translate its exceptions."""
    try:
        ...
        raise UpstreamUnavailableError(
            f"yfinance call failed: {exc}"
        ) from exc


class USStockSource:
    """yfinance-backed data source for US equities."""
    ...
    def get_quote(self, symbol: str) -> Quote:
        sym = normalize_symbol(symbol, Market.US)
        ticker = yf.Ticker(sym)
        info: dict[str, Any] = _yfinance_call(ticker.get_info) or {}
        if not info or info.get("regularMarketPrice") is None and info.get("currentPrice") is None:
            # Fall back to a 1-day history to confirm the symbol exists.
            hist = _yfinance_call(ticker.history, period="5d")
            if hist is None or hist.empty:
                raise SymbolNotFoundError(f"US symbol not found: {sym!r}")
            latest = hist.iloc[-1]
            prev = hist.iloc[-2] if len(hist) >= 2 else latest
            price = float(latest["Close"])
            prev_close = float(prev["Close"])
            ts = latest.name.to_pydatetime() if hasattr(latest.name, "to_pydatetime") else datetime.now(timezone.utc)
            return Quote(
                symbol=sym,
                market=Market.US,
    ...
    def get_ohlcv(
        ...
        if hist is None or hist.empty:
            raise SymbolNotFoundError(f"US symbol not found: {sym!r}")
        return self._df_to_bars(hist.tail(limit))
    ...
    def search_company(self, query: str, limit: int = 5) -> list[SearchResult]:
        ...
        for q_ in quotes[:limit]:
            sym = str(q_.get("symbol", ""))
            # Keep US-only results; HK shares appear as "0700.HK" and we filter them out.
            if not sym or sym.endswith((".HK", ".SS", ".SZ")):
                continue
            results.append(
                SearchResult(
                    symbol=sym,
    ...
    @staticmethod
    def _df_to_bars(df: pd.DataFrame) -> list[OHLCV]:
        bars: list[OHLCV] = []
