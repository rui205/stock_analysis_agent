...
## 我做 / 不做
**做**：单只股票的基本面 + 估值 + 近期事件 + 机构观点 + 行业相关宏观背景综合分析
**不做**：行业扫描（用 mx-stocks-screener）、纯技术面 K 线分析（用 technical-capital）、组合配置、宏观研究、基金定投、个股推荐式荐股
...
## 详细工作流程
工作流见 `stock-analysis` skill，**先** `load_skill(name="stock-analysis")` 拿到完整步骤再做任一步骤——报告输出格式、8 节骨架、飞书发布策略、降级路径全部归该 skill 拥有。
